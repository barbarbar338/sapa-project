 ...
 * @param {Number} [endFreq=20000] - Spectrum end frequency.
 * @returns {{ magnitudes: number[], frequencies: number[] }} - The magnitudes and frequencies.
 */
function CooleyTukeyFFT(signal, sampleRate, startFreq = 20, endFreq = 20000) {
	...
	if (N <= 1)
		return { magnitudes: [Math.abs(signal[0] || 0)], frequencies: [0] };
	if (N % 2 !== 0)
	...
	function recursiveFFT(input) {
		...
		for (let k = 0; k < n / 2; k++) {
			const t = {
				real:
					Math.cos((-2 * Math.PI * k) / n) * odd[k].real -
					Math.sin((-2 * Math.PI * k) / n) * odd[k].imag,
				imag:
					Math.cos((-2 * Math.PI * k) / n) * odd[k].imag +
	...
	const magnitudes = fftResults.map((complex) =>
		Math.sqrt(complex.real ** 2 + complex.imag ** 2),
	);
